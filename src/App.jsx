      ...
      <div style={{background:T.bg2,border:`1px solid ${T.bdrA}`,borderRadius:18,padding:"32px 28px",maxWidth:480,width:"100%",boxShadow:`0 20px 60px rgba(0,0,0,.7)`,animation:"modalIn .3s ease"}}>
        ...
        <div style={{marginBottom:18}}>
          ...
          <select
            ...
            style={{width:"100%",background:T.bg3,border:`1px solid ${T.bdr}`,borderRadius:10,padding:"11px 14px",color:T.t1,fontSize:13,outline:"none"}}
          >
            {AVAILABLE_MODELS.map(m=>(
              <option key={m.id} value={m.id} style={{background:T.bg3,color:T.t1}}>{m.name}</option>
            ))}
          </select>
          {isCustom && (
            <input
              type="text"
              placeholder="e.g. gemini-2.0-flash-exp or tunedModels/..."
              value={customModel}
              onChange={e=>setCustomModel(e.target.value)}
              style={{width:"100%",background:T.bg3,border:`1px solid ${T.bdr}`,borderRadius:10,padding:"10px 14px",color:T.t1,fontSize:12,marginTop:8}}
            />
          )}
        </div>
        ...
        {/* API Key */}
        <div style={{marginBottom:18}}>
          <label style={{fontSize:11,fontWeight:700,color:T.indL,letterSpacing:1,display:"block",marginBottom:6,textTransform:"uppercase"}}>API Key (Google AI Studio)</label>
          <input 
            type="password" 
            value={keyInput} 
            onChange={e=>setKeyInput(e.target.value)} 
            placeholder="Paste AIzaSy..." 
            style={{width:"100%",background:T.bg3,border:`1px solid ${T.bdr}`,borderRadius:10,padding:"12px 14px",color:T.t1,fontSize:13}}
          />
        </div>

        <div style={{marginBottom:24}}>
          <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" style={{color:T.tel,fontSize:12,textDecoration:"underline",fontWeight:600}}>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          ...
          <Btn v="gold" onClick={()=>{ 
            ...
            onSave(keyInput.trim(), finalModel || "gemini-2.5-flash"); 
            onClose(); 
          }}>Save Settings</Btn>
        </div>
      </div>
...
function Home({setScreen,player,showTut,setShowTut,openApiKeyModal,hasKey,currentModel}){
  const bp=useBP();
  ...
  return(
    <div style={{minHeight:"100vh",fontFamily:F,color:T.t1,background:`radial-gradient(ellipse 60% 45% at 15% 12%,${T.ind}08 0%,transparent 55%),radial-gradient(ellipse 50% 40% at 85% 88%,${T.gld}05 0%,transparent 55%),${T.bg0}`}}>
      ...
      <nav style={{position:"sticky",top:0,zIndex:50,padding:bp.mob?"12px 16px":"14px 44px",borderBottom:`1px solid ${T.bdr}`,backdropFilter:"blur(20px)",background:"rgba(5,8,15,.76)",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <Btn v={hasKey?"ghost":"gold"} sz="sm" onClick={openApiKeyModal}>
            {hasKey ? `⚡ ${currentModel}` : "⚠️ Set Gemini Key"}
          </Btn>
      ...
      <div style={{textAlign:"center",padding:bp.mob?"44px 16px 32px":"72px 44px 52px",animation:"fadeUp .55s ease"}}>
        <Badge color={T.ind}>Running {currentModel}</Badge>
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      const savedModel = localStorage.getItem("casecraft_gemini_model") || "gemini-2.5-flash";
  ...
  function handleSaveSettings(key, model) {
    if(key) {
    ...
    if(model) {
      localStorage.setItem("casecraft_gemini_model", model);
  ...
  return (
    <>
      ...
      {screen==="home"          && <Home setScreen={setScreen} player={player} showTut={showTut} setShowTut={(v)=>{ if(v) setShowTut(true); else dismissTutorial(); }} openApiKeyModal={()=>setShowKeyModal(true)} hasKey={hasKey} currentModel={currentModel}/>}
    </>
  );
}
